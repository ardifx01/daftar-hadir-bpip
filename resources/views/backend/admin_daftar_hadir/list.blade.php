    ...
    <div class="col-md-12">
      <div class="card">
        ...
        <div class="card-body">
          <div class="row">
            <div class="col-md-8">
                {!! Form::select('param', $arr_satuan_kerja, $satuan_kerja_id, array('class' => 'form-control', 'id'=>'cmd-change-satuan-kerja')) !!}
            </div>
          </div>
          <br>
          <div class="buttons">
            {{-- <a href="/kegiatan/create" class="btn btn-icon icon-left btn-primary"><i class="far fa-edit"></i>Buat Daftar Hadir Kegiatan</a> --}}
            ...
            <table class="table table-striped" id="table-kegiatan">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td style="font-size: 12px">{{ $item_kegiatan->judul_kegiatan}}<br><b>Created by : </b>{{ $item_kegiatan->usernameintra}}</td>
                  <td style="font-size: 12px">{{ $item_kegiatan->deskripsi_kegiatan}}</td>
                  <td style="font-size: 12px">{{ $item_kegiatan->lokasi_kegiatan}}</td>
                  <td style="font-size: 12px" class="text-center">{{ $item_kegiatan->satuan_kerja_text}}</td>
...
<script>
  $(document).ready(function() {
    ...
    $('#cmd-change-satuan-kerja').on('change', function(e){
        event.preventDefault();

        let satuan_kerja_id = $(this).val();
      //  alert(satuan_kerja_id);
        window.location.href = `{{ route('admin-daftar-hadir.list-all') }}/${satuan_kerja_id}`;

        // $.ajax({
        //     url: "{{ route('admin-daftar-hadir.list-all', "+ satuan_kerja_id +")}}",
        //     type: "GET",
        //     dataType: "json",
        //     success: function(result) {
